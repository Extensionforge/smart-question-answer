<?php
/**
 * SmartQa Input type field object.
 *
 * @package    SmartQa
 * @subpackage Fields
 * @since      4.1.0
...
class Input extends Field {
	...
	protected function prepare() {
		$this->args = wp_parse_args(
			...
			array(
				'subtype' => 'text',
				'label'   => __( 'SmartQa Input Field', 'smart-question-answer' ),
			)
		);
	...
	protected function html_order() {
		...
		if ( $this->get( 'html' ) && empty( $this->args['label'] ) ) {
			$this->output_order = array_diff( $this->output_order, array( 'label', 'field_wrasqa_start', 'field_wrasqa_end' ) );
		}
	}
