<?php
/**
 * SmartQa Checkbox type field object.
 *
 * @package    SmartQa
 * @subpackage Fields
 * @since      4.1.0
...
class Checkbox extends Field {
	...
	protected function prepare() {
		$this->args = wp_parse_args(
			$this->args,
			array(
				'label' => __( 'SmartQa Checkbox Field', 'smart-question-answer' ),
			)
		);
	...
	protected function html_order() {
		...
		if ( ! $this->get( 'options' ) ) {
			$this->output_order = array( 'wrapper_start', 'label', 'field_wrasqa_start', 'errors', 'field_markup', 'field_wrasqa_end', 'wrapper_end' );
		}
	}
	...
	public function field_markup() {
		...
		if ( $this->get( 'options' ) ) {
			...
			foreach ( $this->get( 'options' ) as $val => $label ) {
				...
				$this->add_html( '<label>' );
				$this->add_html( '<input type="checkbox" value="1" name="' . esc_attr( $this->field_name ) . '[' . $val . ']" id="' . sanitize_html_class( $this->field_name . $val ) . '" class="asqa-form-control" ' . $checked . $this->custom_attr() . '/>' );
