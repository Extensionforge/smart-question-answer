...
class Field {
	...
	public function form() {
		$form_name = explode( '.', asqa_to_dot_notation( $this->form_name ) );
		return smartqa()->get_form( $form_name[0] );
	}
	...
	protected function html_order() {
		if ( empty( $this->args['output_order'] ) ) {
			$this->output_order = array( 'wrapper_start', 'label', 'field_wrasqa_start', 'errors', 'field_markup', 'desc', 'field_wrasqa_end', 'wrapper_end' );
		} else {
	...
	public function output() {
		 ...
		 */
		$this->html = apply_filters( 'asqa_field_html', $this->html, $this ); // xss okay.
	 ...
	 * @return void
	 */
	protected function field_wrasqa_start() {
		$this->add_html( '<div class="asqa-field-group-w">' );
	}
	 ...
	 * @return void
	 */
	protected function field_wrasqa_end() {
		$this->add_html( '</div>' );
	}
	...
	public function label() {
		$this->add_html( '<label class="asqa-form-label" for="' . $this->id() . '">' . esc_html( $this->get( 'label' ) ) . '</label>' );
	}
	...
	public function add_error( $code, $message = '' ) {
		...
		$name                  = explode( '.', asqa_to_dot_notation( $this->form_name ) );

		if ( is_array( $name ) ) {
			smartqa()->get_form( $name[0] )->add_error( 'fields-error', __( 'Error found in fields, please check and re-submit', 'smart-question-answer' ) );
		}
	}
	...
	public function errors() {
		...
		if ( false !== $wrapper ) {
			$this->add_html( '<div class="asqa-field-errorsc">' );
		}

		if ( $this->have_errors() ) {
			$this->add_html( '<div class="asqa-field-errors">' );
			foreach ( $this->errors as $code => $err ) {
				$this->add_html( '<span class="asqa-field-error ecode-' . esc_attr( $code ) . '">' . esc_html( $err ) . '</span>' );
			}
			$this->add_html( '</div>' );
		}
	...
	public function field_markup() {
		 ...
		 * @since 4.1.0
		 */
		do_action_ref_array( 'asqa_before_field_markup', array( $this ) );
	}
	...
	public function desc() {
		if ( $this->get( 'desc' ) ) {
			$this->add_html( '<div class="asqa-field-desc">' . wp_kses_post( $this->get( 'desc' ) ) . '</div>' );
		}
	}
	...
	protected function wrapper_start() {
		...
		if ( false !== $wrapper ) {
			$errors = $this->have_errors() ? ' asqa-have-errors' : '';

			$this->add_html( '<div class="asqa-form-group asqa-field-' . $this->id() . ' asqa-field-type-' . esc_attr( $this->type ) . $errors . ' ' . esc_attr( $this->get( 'wrapper.class', '' ) ) . '"' . $this->get_attr( $this->get( 'wrapper.attr' ) ) . '>' );
		}
	}
	...
	protected function common_attr() {
		return ' name="' . esc_attr( $this->field_name ) . '" id="' . $this->id() . '" class="asqa-form-control ' . esc_attr( $this->get( 'class', '' ) ) . '"';
	}
	...
	public function sanitize() {
		...
		if ( ! empty( $this->sanitize_cb ) ) {
			...
			foreach ( (array) $this->sanitize_cb as $sanitize ) {
				...
				if ( method_exists( 'SmartQa\Form\Validate', $cb ) ) {
					$sanitized_val = call_user_func_array( 'SmartQa\Form\Validate::' . $cb, $this->sanitize_cb_args( $unsafe_value ) );
	...
	public function validate() {
		...
		if ( ! empty( $this->validate_cb ) ) {
			foreach ( (array) $this->validate_cb as $validate ) {
				...
				if ( method_exists( 'SmartQa\Form\Validate', $cb ) ) {
					call_user_func_array( 'SmartQa\Form\Validate::' . $cb, array( $this ) );
				}
				 ...
				 * @since 4.1.7
				 */
				apply_filters_ref_array( "asqa_field_{$cb}", array( $this->field_name, $this ) );
			}
