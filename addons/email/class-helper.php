...
namespace SmartQa\Addons\Email;
...
class Helper {
	...
	public function add_email( $email ) {
		...
		if ( ! in_array( $email, $this->emails, true ) ) {
			 ...
			 * @since 4.1.0
			 */
			do_action_ref_array( 'asqa_before_email_to_list', array( $this ) );
	...
	public function add_template_tag( $tag, $content ) {
		...
		if ( ! isset( $this->template_tags[ $tag ] ) ) {
			 ...
			 * @since 4.1.0
			 */
			do_action_ref_array( 'asqa_adding_email_tag', array( $this ) );
		}
	}
	...
	public function get_default_template() {
		 ...
		 * @since 4.1.0
		 */
		return apply_filters( "asqa_email_default_template_{$this->event}", '' );
	}
	...
	public function prepare_template() {
		$page_id = asqa_opt( 'email_template_' . $this->event );
		...
		if ( empty( $this->template ) ) {
			$default_template = \SmartQa\Addons\Email::init()->get_default_template( $this->event );
		...
		$main_tags = array(
			...
			'style'       => file_get_contents( asqa_get_theme_location( 'addons/email/style.css' ) ), // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
			'site_name'   => get_bloginfo( 'name' ),
		);
		 ...
		 * @return string
		 */
		$main_tags = apply_filters_ref_array( 'asqa_email_main_tags', array( $main_tags, $this ) );

		foreach ( $main_tags as $key => $content ) {
			$this->add_template_tag( $key, $content );
		}

		$main_template = file_get_contents( asqa_get_theme_location( 'addons/email/template.html' ) ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
		 ...
		 * @return string
		 */
		$main_template = apply_filters_ref_array( 'asqa_email_template_prepared', array( $main_template, $this ) );
	...
	public function send_emails() {
		if ( defined( 'ASQA_DISABLE_EMAIL' ) && true === ASQA_DISABLE_EMAIL ) {
			return;
		}
