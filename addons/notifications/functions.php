<?php
/**
 * SmartQa notification functions.
 *
 * @package   SmartQa
 ...
 */
function asqa_insert_notification( $args = array() ) {

	// Dont do insert notification if defined.
	if ( defined( 'ASQA_DISABLE_INSERT_NOTI' ) && ASQA_DISABLE_INSERT_NOTI ) {
		return;
	}
	...
	global $wpdb;
	$exists = asqa_get_notifications( $noti_args );

	// If already exists then just update date and mark as unread.
	if ( ! empty( $exists ) ) {
		return $wpdb->update(
			$wpdb->prefix . 'asqa_notifications',
			array(
 ...
 * @param array $args Arguments.
 */
function asqa_get_notifications( $args = array() ) {
	global $wpdb;
	...
	$query = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}asqa_notifications WHERE noti_user_id = %d {$actor_q} {$ref_parent_q} {$ref_id_q} {$ref_type_q} {$verb_q} {$seen_q} LIMIT {$offset},{$number}", $args['user_id'] ); // phpcs:ignore WordPress.DB
 ...
 * @param array $args Arguments.
 */
function asqa_delete_notifications( $args = array() ) {
	global $wpdb;
	$where = array();
	...
	$delete      = $wpdb->query( "DELETE FROM {$wpdb->prefix}asqa_notifications WHERE 1=1 {$where_claue}" ); // phpcs:ignore WordPress.DB

	if ( false === $delete ) {
		return $delete;
	}

	do_action( 'asqa_deleted_notifications', $args );
 ...
 */
function asqa_set_notification_as_seen( $noti_id ) {
	global $wpdb;
 ...
 */
function asqa_set_notifications_as_seen( $user_id ) {
	global $wpdb;
	return $wpdb->update(
		$wpdb->prefix . 'asqa_notifications',
		array(
 ...
 */
function asqa_register_notification_verb( $key, $args = array() ) {
	global $asqa_notification_verbs;
	...
	$asqa_notification_verbs[ $key ] = $args;
}

/**
 * Get all SmartQa notification verbs.
 *
 * @since unknown
 */
function asqa_notification_verbs() {
	global $asqa_notification_verbs;

	if ( empty( $asqa_notification_verbs ) ) {
		do_action( 'asqa_notification_verbs' );
	}

	return $asqa_notification_verbs;
}
 ...
 * @return integer
 */
function asqa_count_unseen_notifications( $user_id = false ) {
	if ( false === $user_id ) {
		$user_id = get_current_user_id();
	}
	...
	$count = (int) $wpdb->get_var( $wpdb->prepare( "SELECT count(*) FROM {$wpdb->prefix}asqa_notifications WHERE noti_user_id = %d AND noti_seen = 0", $user_id ) ); // phpcs:ignore WordPress.DB
