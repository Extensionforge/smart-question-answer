<?php
/**
 * Holds all hooks related to SmartQa forms.
 *
...
class ASQA_Form_Hooks {
	...
	public static function question_form() {
		...
		$form = array(
			'submit_label' => __( 'Submit Question', 'smart-question-answer' ),
			'fields'       => array(
				'post_title'   => array(
					'type'       => 'input',
					'label'      => __( 'Title', 'smart-question-answer' ),
					'desc'       => __( 'Question in one sentence', 'smart-question-answer' ),
					'attr'       => array(
						'autocomplete'   => 'off',
						'placeholder'    => __( 'Question title', 'smart-question-answer' ),
						'data-action'    => 'suggest_similar_questions',
						'data-loadclass' => 'q-title',
					),
					'min_length' => asqa_opt( 'minimum_qtitle_length' ),
					'max_length' => 100,
				...
				'post_content' => array(
					'type'        => 'editor',
					'label'       => __( 'Description', 'smart-question-answer' ),
					'min_length'  => asqa_opt( 'minimum_question_length' ),
		...
		if ( asqa_opt( 'allow_private_posts' ) ) {
			$form['fields']['is_private'] = array(
				'type'  => 'checkbox',
				'label' => __( 'Is private?', 'smart-question-answer' ),
				'desc'  => __( 'Only visible to admin and moderator.', 'smart-question-answer' ),
			);
		}

		// Add name fields if anonymous is allowed.
		if ( ! is_user_logged_in() && asqa_allow_anonymous() ) {
			$form['fields']['anonymous_name'] = array(
				'label'      => __( 'Your Name', 'smart-question-answer' ),
				'attr'       => array(
					'placeholder' => __( 'Enter your name to display', 'smart-question-answer' ),
				),
				'order'      => 20,
			...
			if ( empty( $editing_id ) && asqa_opt( 'create_account' ) ) {
				$form['fields']['email'] = array(
					'label'      => __( 'Your Email', 'smart-question-answer' ),
					'attr'       => array(
						'placeholder' => __( 'Enter your email', 'smart-question-answer' ),
					),
		...
		$post_parent = asqa_isset_post_value( 'post_parent', false );
		if ( $post_parent && wp_verify_nonce( asqa_isset_post_value( '__nonce_pp' ), 'post_parent_' . $post_parent ) ) {
		 ...
		 * @since   4.1.0
		 */
		$form = apply_filters( 'asqa_question_form_fields', $form, $editing );
	...
	public static function answer_form() {
		$editing     = false;
		$editing_id  = asqa_sanitize_unslash( 'id', 'r' );
		$question_id = asqa_sanitize_unslash( 'question_id', 'r', get_question_id() );

		$form = array(
			'submit_label' => __( 'Post Answer', 'smart-question-answer' ),
			'fields'       => array(
				'post_content' => array(
					'type'        => 'editor',
					'label'       => __( 'Description', 'smart-question-answer' ),
					'min_length'  => asqa_opt( 'minimum_ans_length' ),
					'validate'    => 'required,min_string_length,badwords',
					'editor_args' => array(
						'quicktags' => asqa_opt( 'question_text_editor' ) ? true : false,
					),
		 ...
		 * @since   4.1.0
		 */
		return apply_filters( 'asqa_answer_form_fields', $form, $editing );
	}
	...
	public static function comment_form() {
		$form = array(
			'submit_label' => __( 'Submit Comment', 'smart-question-answer' ),
			'fields'       => array(
				'content' => array(
					'type'        => 'textarea',
					'label'       => __( 'Comment', 'smart-question-answer' ),
					'min_length'  => 5,
					'validate'    => 'required,min_string_length,badwords',
					'attr'        => array(
						'placeholder' => __( 'Write your comment here...', 'smart-question-answer' ),
						'rows'        => 5,
					),
					'editor_args' => array(
						'quicktags'     => true,
		...
		if ( ! is_user_logged_in() ) {
			...
			$form['fields']['email'] = array(
				'label'      => __( 'Your Email', 'smart-question-answer' ),
				'attr'       => array(
					'placeholder' => __( 'Enter your email to get follow up notifications.', 'smart-question-answer' ),
				),
				'subtype'    => 'email',
			...
			$form['fields']['url'] = array(
				'label'      => __( 'Your Website', 'smart-question-answer' ),
				'attr'       => array(
					'placeholder' => __( 'Enter link to your website.', 'smart-question-answer' ),
				),
				'subtype'    => 'url',
				'validate'   => 'is_url',
				'max_length' => 254,
		 ...
		 * @since   4.1.0
		 */
		$form = apply_filters( 'asqa_comment_form_fields', $form );
	...
	public static function submit_question_form( $manual = false ) {
		 ...
		 * @since 4.1.0
		 */
		do_action( 'asqa_submit_question_form' );
		...
		if ( ! empty( $values['post_id']['value'] ) ) {
			...
			$editing             = true;
			$_post               = asqa_get_post( $question_args['ID'] );

			// Check if valid post type and user can edit.
			if ( false !== $manual && ( 'question' !== $_post->post_type || ! asqa_user_can_edit_question( $_post ) ) ) {
		...
		// Post status.
		$question_args['post_status'] = asqa_new_edit_post_status( false, 'question', $editing );
		...
		if ( $form->have_errors() ) {
			if ( false === $manual ) {
				asqa_ajax_json(
					array(
						'success'       => false,
						'snackbar'      => array( 'message' => __( 'Unable to post question.', 'smart-question-answer' ) ),
						'form_errors'   => $form->errors,
						'fields_errors' => $form->get_fields_errors(),
					)
				);
			} else {
				return new WP_Error( 'failed', __( 'Failed to insert question', 'smart-question-answer' ) );
			}
		}

		// Set post parent.
		$post_parent = asqa_sanitize_unslash( 'post_parent', 'r' );
		if ( ! empty( $post_parent ) && wp_verify_nonce( asqa_sanitize_unslash( '__nonce_pp', 'r' ), 'post_parent_' . $post_parent ) ) {
		...
		if ( ! $editing && ! is_user_logged_in() && asqa_opt( 'create_account' ) ) {
			self::create_user( $values, $question_args, $manual );
		}

		// Check if duplicate.
		if ( ! $editing && asqa_opt( 'duplicate_check' ) && false !== asqa_find_duplicate_post( $question_args['post_content'], 'question' ) ) {
			$form->add_error( 'duplicate-question', __( 'You are trying to post a duplicate question. Please search existing questions before posting a new one.', 'smart-question-answer' ) );
		 ...
		 */
		$question_args['post_content'] = apply_filters( 'asqa_form_contents_filter', $question_args['post_content'] );

		$question_args['post_name'] = asqa_remove_stop_words_post_name( $question_args['post_title'] );
		...
		if ( $editing ) {
			 ...
			 */
			$question_args = apply_filters( 'asqa_pre_update_question', $question_args );
		} else {
			/**
			 ...
			 * @since 4.1.0 Moved from includes/ask-form.php.
			 */
			$question_args = apply_filters( 'asqa_pre_insert_question', $question_args );
		}
					...
					array(
						...
						'snackbar' => array(
							'message' => sprintf(
								...
								__( 'Unable to post question. Error: %s', 'smart-question-answer' ),
		 ...
		 * @since 4.1.11
		 */
		do_action( 'asqa_after_question_form_processed', $post_id );

		if ( isset( $question_args['ID'] ) ) {
			$message = __( 'Question updated successfully, you\'ll be redirected in a moment.', 'smart-question-answer' );
		} else {
			$message = __( 'Your question is posted successfully, you\'ll be redirected in a moment.', 'smart-question-answer' );
		}


		if ( is_user_logged_in() ) {
		   $asqaredirect = get_permalink( $post_id );
		} else {

		   $asqaredirect = "#";
		   $message = __( 'Your question is posted successfully, please login or register, confirm and login.', 'smart-question-answer' );

		    if (!isset($_COOKIE['asqa_guest_cookie'])) {
		        setcookie('asqa_guest_cookie', $post_id, time()+86400,"/");
		    } else {
		    	$cookieValue = $_COOKIE['asqa_guest_cookie'];
		    	$neuewerte = $cookieValue.",".$post_id;
		        setcookie('asqa_guest_cookie', $neuewerte, time()+86400,"/");
		...
		if ( false === $manual ) {
			smartqa()->session->set_question( $post_id );
			...
			asqa_ajax_json(
				array(
					...
					'redirect' => $asqaredirect,
					'post_id'  => $post_id,
					'frontpagewidget'  => true,
				)
			);
		}
	...
	public static function submit_answer_form( $manual = false ) {
		$editing     = false;
		$question_id = asqa_sanitize_unslash( 'question_id', 'r' );
		 ...
		 * @since 4.1.0
		 */
		do_action( 'asqa_submit_answer_form' );
		...
		if ( ! empty( $values['post_id']['value'] ) ) {
			...
			$editing           = true;
			$_post             = asqa_get_post( $answer_args['ID'] );

			// Check if valid post type and user can edit.
			if ( 'answer' !== $_post->post_type || ! asqa_user_can_edit_answer( $_post ) ) {
				asqa_ajax_json( 'something_wrong' );
			}
		}
		...
		// Post status.
		$answer_args['post_status'] = asqa_new_edit_post_status( false, 'answer', $editing );
		...
		if ( $form->have_errors() ) {
			if ( false === $manual ) {
				...
				return new WP_Error( 'failed', __( 'Please check field', 'smart-question-answer' ) );
			}
		}
		 ...
		 * @since @3.0.0 Moved from process-form.php
		 */
		$answer_args['post_content'] = apply_filters( 'asqa_form_contents_filter', $answer_args['post_content'] );

		$answer_args['post_name'] = asqa_remove_stop_words_post_name( $answer_args['post_title'] );
		...
		if ( $editing ) {
			 ...
			 */
			$answer_args = apply_filters( 'asqa_pre_update_answer', $answer_args );
		} else {
			/**
			 ...
			 * @since 4.1.0 Moved from includes/answer-form.php.
			 */
			$answer_args = apply_filters( 'asqa_pre_insert_answer', $answer_args );
		}
				...
				asqa_ajax_json(
					array(
						...
						'snackbar' => array(
							'message' => sprintf(
								// Translators: placeholder contain error message.
								__( 'Unable to post answer. Error: %s', 'smart-question-answer' ),
								$post_id->get_error_message()
							),
						),
					)
		 ...
		 * @since 4.1.11
		 */
		do_action( 'asqa_after_answer_form_processed', $post_id );

		if ( ! $editing ) {
			smartqa()->session->set_answer( $post_id );
			asqa_answer_post_ajax_response( $question_id, $post_id );
		}

		if ( $editing ) {
			$message = __( 'Answer updated successfully. Redirecting you to question page.', 'smart-question-answer' );
		} else {
			$message = __( 'Your answer is posted successfully.', 'smart-question-answer' );
		}
	...
	public static function submit_comment_form() {
		 ...
		 * @since 4.1.0
		 */
		do_action( 'asqa_submit_comment_form' );

		$values  = $form->get_values();
		$post_id = asqa_sanitize_unslash( 'post_id', 'r' );

		// Check nonce and is valid form.
		if ( ! $form->is_submitted() || ! asqa_user_can_comment( $post_id ) ) {
			asqa_ajax_json(
				array(
					'success'  => false,
					'snackbar' => array( 'message' => __( 'Trying to cheat?!', 'smart-question-answer' ) ),
		...
		if ( $form->have_errors() ) {
			asqa_ajax_json(
				array(
					'success'       => false,
					'snackbar'      => array( 'message' => __( 'Unable to post comment.', 'smart-question-answer' ) ),
					'form_errors'   => $form->errors,
					'fields_errors' => $form->get_fields_errors(),
				)
			);
		...
		if ( ! empty( $comment_id ) ) {
			...
			if ( 'smartqa' !== $comment->comment_type || ! asqa_user_can_edit_comment( $comment_id ) ) {
				asqa_ajax_json(
					array(
						'success'  => false,
						'snackbar' => array( 'message' => __( 'You cannot edit this comment.', 'smart-question-answer' ) ),
					)
				);
			}
			...
			if ( $values['content']['value'] === $comment->comment_content ) {
				asqa_ajax_json(
					array(
						'success'  => false,
						'snackbar' => array( 'message' => __( 'There is no change in your comment.', 'smart-question-answer' ) ),
					)
				);
			}
			...
			if ( ! is_wp_error( $updated ) ) {
				 ...
				 */
				do_action( 'asqa_edit_comment', $comment_id );
				...
				$result = array(
					...
					'commentsCount' => array(
						...
						'text'       => sprintf( _n( '%d Comment', '%d Comments', $count['all'], 'smart-question-answer' ), $count['all'] ),
						'number'     => $count['all'],
						'unapproved' => $count['awaiting_moderation'],
					),
					'snackbar'      => array( 'message' => __( 'Comment updated successfully', 'smart-question-answer' ) ),
					'html'          => $html,
				...
				array(
					...
					'snackbar' => array(
						'message' => sprintf(
							...
							__( 'Commenting is not allowed on draft, pending or deleted %s', 'smart-question-answer' ),
							$type
						),
					),
				)
		 ...
		 * @since 3.0.0
		 */
		$commentdata = apply_filters( 'asqa_pre_insert_comment', $commentdata );
		...
		if ( ! is_wp_error( $comment_id ) && false !== $comment_id ) {
			$c = get_comment( $comment_id );
			do_action( 'asqa_after_new_comment', $c );

			$count = get_comment_count( $c->comment_post_ID );

			ob_start();
			asqa_the_comments( $c->comment_post_ID );
			$html = ob_get_clean();
			...
			$result = array(
				...
				'snackbar'      => array( 'message' => __( 'Comment successfully posted', 'smart-question-answer' ) ),
				'html'          => $html,
				'post_id'       => $c->comment_post_ID,
				'comment_id'    => $c->comment_ID,
				'hide_modal'    => 'comment',
	 ...
	 * @param \SmartQa\Field\Upload $field SmartQa field object.
	 * @return array
	 * @since 4.1.8
	 */
	 ...
	 * Image upload form.
	 *
	 * This form is used for uploading images in SmartQa.
	 *
	 * @return array
	 * @since 4.1.8
	 */
	public static function image_upload_form() {
		return array(
			'submit_label' => __( 'Upload & insert', 'smart-question-answer' ),
			'fields'       => array(
				'image' => array(
					'label'          => __( 'Image', 'smart-question-answer' ),
					'desc'           => __( 'Select image(s) to upload. Only .jpg, .png and .gif files allowed.', 'smart-question-answer' ),
					'type'           => 'upload',
					'save'           => array( __CLASS__, 'image_upload_save' ),
					'upload_options' => array(
						'multiple'      => false,
