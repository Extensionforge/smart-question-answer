<?php
/**
 * SmartQa answer loop related functions and classes
 *
 * @package      SmartQa
...
class Answers_Query extends WP_Query {
	...
	public function __construct( $args = array() ) {
		global $answers;
		$paged    = (int) max( 1, get_query_var( 'asqa_paged', 1 ) );
		$defaults = array(
			'question_id'            => get_question_id(),
			'asqa_query'               => true,
			'asqa_current_user_ignore' => false,
			'asqa_answers_query'       => true,
			'showposts'              => asqa_opt( 'answers_per_page' ),
			'paged'                  => $paged,
			'only_best_answer'       => false,
			'ignore_selected_answer' => false,
			'post_status'            => array( 'publish' ),
			'asqa_order_by'            => asqa_opt( 'answers_sort' ),
		);
	...
	public function reset_answers_data() {
		...
		if ( ! empty( $this->post ) ) {
			smartqa()->current_answer = $this->post;
		}
	}
	...
	public function get_ids() {
		...
		foreach ( (array) $this->posts as $_post ) {
			...
			$this->asqa_ids['attach_ids'] = array_filter( array_merge( explode( ',', $_post->attach ), $this->asqa_ids['attach_ids'] ) );
	...
	public function pre_fetch() {
		$this->get_ids();
		asqa_prefetch_recent_activities( $this->asqa_ids['post_ids'], 'a_id' );
		...
		do_action( 'asqa_pre_fetch_answer_data', $this->asqa_ids );
	}
}
 ...
 * @since  2.0
 */
function asqa_get_answers( $args = array() ) {
	if ( empty( $args['question_id'] ) ) {
		$args['question_id'] = get_question_id();
	}

	if ( ! isset( $args['asqa_order_by'] ) ) {
		$order_by = asqa_sanitize_unslash( 'order_by', 'g' );

		$args['asqa_order_by'] = ! empty( $order_by ) ? $order_by : asqa_opt( 'answers_sort' );
	}
 ...
 * @since 2.1
 */
function asqa_get_answer( $answer_id ) {
 ...
 * @since  2.0
 */
function asqa_get_best_answer( $question_id = false ) {
 ...
 * @return int
 */
function asqa_total_answers_found() {
	global $answers;
 ...
 * @return object
 */
function asqa_answer_the_object() {
	global $answers;
	if ( ! $answers ) {
		return;
	}
 ...
 * @since 2.1
 */
function asqa_answer_user_can_view() {
	return asqa_user_can_view_post( get_the_ID() );
}
 ...
 * @return void.
 */
function asqa_answers_the_pagination() {
	if ( get_query_var( 'answer_id' ) ) {
		echo '<a class="asqa-all-answers" href="' . esc_url( get_permalink( get_question_id() ) ) . '">' .
		// translators: %d is total answer count of question.
		esc_attr( sprintf( __( 'You are viewing 1 out of %d answers, click here to view all answers.', 'smart-question-answer' ), asqa_get_answers_count( get_question_id() ) ) ) . '</a>';
	} else {
		global $answers;
		$paged = ( get_query_var( 'asqa_paged' ) ) ? get_query_var( 'asqa_paged' ) : 1;
		asqa_pagination( $paged, $answers->max_num_pages, '?asqa_paged=%#%', get_permalink( get_question_id() ) . 'page/%#%/' );
	}
}
 ...
 * @return integer
 */
function asqa_count_published_answers( $question_id ) {
	global $wpdb;
 ...
 * @return int
 */
function asqa_count_other_answer( $question_id = false ) {
	if ( ! $question_id ) {
		$question_id = get_question_id();
	}

	$count = asqa_get_answers_count( $question_id );

	if ( asqa_have_answer_selected( $question_id ) ) {
 ...
 * @since 4.0.0
 */
function asqa_get_answer_position_paged( $question_id = false, $answer_id = false ) {
	global $wpdb;
	...
	$asqa_order_by = asqa_get_current_list_filters( 'order_by', 'active' );

	if ( 'voted' === $asqa_order_by ) {
	...
	} if ( 'oldest' === $asqa_order_by ) {
		$orderby = "{$wpdb->posts}.post_date ASC";
	} elseif ( 'newest' === $asqa_order_by ) {
	...
	$ids = $wpdb->get_col( $wpdb->prepare( "SELECT p.ID FROM $wpdb->posts p LEFT JOIN $wpdb->asqa_qameta qameta ON qameta.post_id = p.ID  WHERE p.post_type = 'answer' AND p.post_parent = %d AND ( $status OR ( p.post_author = %d AND p.post_status IN ('publish', 'private_post', 'trash', 'moderate') ) ) ORDER BY $orderby", $question_id, $user_id ) ); // phpcs:ignore WordPress.DB

	$pos   = (int) array_search( $answer_id, $ids ) + 1; // phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict
	$paged = ceil( $pos / asqa_opt( 'answers_per_page' ) );
 ...
 */
function asqa_answer_status( $_post = null ) {
	asqa_question_status( $_post );
}
