<?php
/**
 * Installation and activation of smartqa, register hooks that are fired when the plugin is activated.
 *
 * @package     SmartQa
...
class ASQA_Activate {
	...
	/**
	 * Disable old SmartQa extensions.
	 */
	public function disable_ext() {
		deactivate_plugins(
			array(
				'categories-for-smartqa/categories-for-smartqa.php',
				'tags-for-smartqa/tags-for-smartqa.php',
				'smartqa-email/smartqa-email.php',
				'question-labels/question-labels.php',
				'smartqa-paid-membership/smartqa-paid-membership.php',
			)
		);
	}

	/**
	 * Delete old SmartQa options.
	 *
	 * @since 4.1.5
	 */
	public function delete_options() {
		$settings = get_option( 'smartqa_opt', array() );
	...
	public function enable_addons() {
		...
		asqa_activate_addon( 'email.php' );
		asqa_activate_addon( 'categories.php' );
	}
	...
	public function qameta_table() {
		...
		$this->tables[] = 'CREATE TABLE ' . $wpdb->asqa_qameta . ' (
	...
	/**
	 * SmartQa asqa_votes table.
	 */
	public function votes_table() {
		global $wpdb;

		$this->tables[] = 'CREATE TABLE ' . $wpdb->asqa_votes . ' (
	...
	public function views_table() {
		...
		$this->tables[] = 'CREATE TABLE ' . $wpdb->asqa_views . ' (
	...
	public function reputation_table() {
		...
		$this->tables[] = 'CREATE TABLE ' . $wpdb->asqa_reputations . ' (
	...
	/**
	 * SmartQa email subscription related table.
	 *
	...
	public function subscribers_table() {
		...
		$this->tables[] = 'CREATE TABLE ' . $wpdb->asqa_subscribers . ' (
	...
	public function activity_table() {
		...
		$activity_exists = $wpdb->get_var( "SHOW TABLES LIKE '$wpdb->asqa_activity'" ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery

		// Check activity table exists.
		if ( $activity_exists === $wpdb->asqa_activity ) {
			// Delete old activity table if exists.
			$existing_table = $wpdb->get_row( "SHOW COLUMNS FROM $wpdb->asqa_activity LIKE 'secondary_user'" ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery

			if ( ! empty( $existing_table ) && 'secondary_user' === $existing_table->Field ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName
				$wpdb->query( "DROP TABLE IF EXISTS $wpdb->asqa_activity" ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery
			}
		}

		$this->tables[] = 'CREATE TABLE ' . $wpdb->asqa_activity . ' (
	...
	/**
	 * SmartQa reputation events table.
	 *
	 * @since 4.3.0
	 */
	public function reputation_events_table() {
		...
		$this->tables[] = 'CREATE TABLE ' . $wpdb->asqa_reputation_events . ' (
	...
	public function activate() {
		...
		$asqa_roles->add_roles();
		$asqa_roles->add_capabilities();

		if ( asqa_opt( 'asqa_version' ) !== ASQA_VERSION ) {
			asqa_opt( 'asqa_installed', 'false' );
			asqa_opt( 'asqa_version', ASQA_VERSION );
		}

		if ( get_option( 'smartqa_db_version' ) === '' ) {
			update_option( 'smartqa_using_previous', 'true' );
		}

		$this->insert_tables();
		update_option( 'smartqa_db_version', SMARTQA_DB_VERSION );
		update_option( 'smartqa_opt', get_option( 'smartqa_opt' ) + asqa_default_options() );
	...
	public function reactivate_addons() {
		...
		foreach ( $active_addons as $file => $active ) {
			if ( false !== strpos( $file, 'free/' ) ) {
				...
				asqa_activate_addon( $new_addon_name );
			}
		}
	}
}
