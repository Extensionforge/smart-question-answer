 ...
 * Akismet check.
 *
 * An SmartQa add-on to check for spam in posts before publishing.
 *
 ...
 * @since 1.0.01
 */
class Akismet extends \SmartQa\Singleton {
	...
	protected function __construct() {
		...
		asqa_add_default_options( array( 'spam_post_action' => 'moderate' ) );

		smartqa()->add_filter( 'asqa_settings_menu_features_groups', $this, 'add_to_settings_page' );
		smartqa()->add_filter( 'asqa_form_options_features_akismet', $this, 'option_form' );
		smartqa()->add_action( 'asqa_after_question_form_processed', $this, 'new_question_answer' );
		smartqa()->add_action( 'asqa_after_answer_form_processed', $this, 'new_question_answer' );
		smartqa()->add_action( 'admin_action_asqa_mark_spam', $this, 'submit_spam' );
		smartqa()->add_action( 'post_row_actions', $this, 'row_actions', 10, 2 );
	}
	...
	public function add_to_settings_page( $groups ) {
		$groups['akismet'] = array(
			'label' => __( 'Akismet', 'smart-question-answer' ),
		);
		...
		$form = array(
			...
			'fields'       => array(
				'spam_post_action' => array(
					'label'   => __( 'What to do when post is a spam?', 'smart-question-answer' ),
					'type'    => 'select',
					'options' => array(
						'moderate' => __( 'Change status to moderate', 'smart-question-answer' ),
						'trash'    => __( 'Trash the post', 'smart-question-answer' ),
					),
	...
	private function api_request( $post_id, $submit = false ) {
		...
		$akismet_ua = sprintf( 'WordPress/%s | SmartQa/%s', $GLOBALS['wp_version'], ASQA_VERSION );
		...
		if ( 'true' === $response['body'] || 'Thanks for making the web a better place.' === $response['body'] ) {
			...
			update_post_meta( $post_id, '__asqa_spam', current_time( 'timestamp' ) ); // phpcs:ignore
		}
	}
	...
	public function submit_spam() {
		...
		// Check role.
		if ( ! current_user_can( 'manage_options' ) || ! wp_verify_nonce( asqa_sanitize_unslash( 'nonce', 'r' ), 'send_spam' ) ) {
			die( esc_attr__( 'Cheating', 'smart-question-answer' ) );
		}
	...
	public function row_actions( $actions, $post ) {
		if ( ! asqa_is_cpt( $post ) || 'moderate' === $post->post_status ) {
			return $actions;
		}

		$nonce = wp_create_nonce( 'send_spam' );

		$actions['report_spam'] = '<a href="' . admin_url( 'admin.php?action=asqa_mark_spam&post_id=' . $post->ID . '&nonce=' . $nonce ) . '" aria-label="' . __( 'Mark this post as a spam', 'smart-question-answer' ) . '">' . __( 'Mark as spam', 'smart-question-answer' ) . '</a>';
