<?php
/**
 * SmartQa notification query class.
 *
 * @package    SmartQa
 ...
 * @since 1.0.0
 */
class Notifications extends \SmartQa_Query {
	...
	public function __construct( $args = array() ) {
		...
		$this->verbs             = asqa_notification_verbs();
	...
	public function query() {
		...
		$query = $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}asqa_notifications WHERE noti_user_id = %d {$ref_id_q} {$ref_type_q} {$verb_q} {$seen_q} ORDER BY noti_date {$order} LIMIT {$this->offset},{$this->per_page}", $this->args['user_id'] ); // phpcs:ignore WordPress.DB

		$this->objects = $wpdb->get_results( $query ); // phpcs:ignore WordPress.DB

		$count_query = $wpdb->prepare( "SELECT count(noti_id) FROM {$wpdb->prefix}asqa_notifications WHERE noti_user_id = %d {$ref_id_q} {$ref_type_q} {$verb_q} {$seen_q}", $this->args['user_id'] ); // phpcs:ignore WordPress.DB

		$this->total_count = $wpdb->get_var( apply_filters( 'asqa_notifications_found_rows', $count_query, $this ) ); // phpcs:ignore WordPress.DB
	...
	public function prefetch_actors() {
		...
		asqa_post_author_pre_fetch( $this->ids['user'] );
	}
	...
	public function prefetch_reputations() {
		...
		$reputations = $wpdb->get_results( "SELECT rep_id, rep_event FROM {$wpdb->asqa_reputations} WHERE rep_id in ({$ids})" ); // phpcs:ignore WordPress.DB

		foreach ( (array) $reputations as $rep ) {
			$rep->points = asqa_get_reputation_event_points( $rep->rep_event );
	...
	public function item_template() {
		...
		$file = asqa_get_theme_location( 'addons/notification/' . sanitize_file_name( $verb['ref_type'] . '.php' ) );
	...
	public function get_permalink() {
		if ( in_array( $this->get_ref_type(), array( 'question', 'answer', 'post' ), true ) ) {
			return asqa_get_short_link( array( 'asqa_p' => $this->get_ref_id() ) );
		} elseif ( 'comment' === $this->get_ref_type() ) {
			return asqa_get_short_link( array( 'asqa_c' => $this->get_ref_id() ) );
		} elseif ( 'reputation' === $this->get_ref_type() ) {
			return asqa_get_short_link(
				array(
					'asqa_u' => $this->object->noti_user_id,
					'sub'  => 'reputations',
				)
			);
		}
	...
	public function get_actor() {
		if ( ! $this->hide_actor() ) {
			return asqa_user_display_name( $this->object->noti_actor ); // xss okay.
		} else {
			return __( 'Someone', 'smart-question-answer' );
		}
	}
	...
	public function get_verb() {
		...
		if ( isset( $this->verbs[ $key ] ) ) {
			...
			$args = array(
				'%cpt%'    => __( 'post', 'smart-question-answer' ),
			...
			if ( isset( $this->object->ref ) ) {
				if ( isset( $this->object->ref->post_type ) ) {
					if ( 'question' === $this->object->ref->post_type ) {
						$args['%cpt%'] = __( 'question', 'smart-question-answer' );
					} elseif ( 'answer' === $this->object->ref->post_type ) {
						$args['%cpt%'] = __( 'answer', 'smart-question-answer' );
					}
				}
	...
	public function the_date() {
		echo wp_kses_post( asqa_human_time( $this->get_date(), false ) );
	}
	...
	public function get_ref_title() {
		if ( isset( $this->object->ref ) ) {
			...
			if ( in_array( $verb_args['ref_type'], array( 'post', 'comment' ), true ) && isset( $this->object->ref->post_title ) ) {
				return asqa_truncate_chars( $this->object->ref->post_title, 80 );
			}
		}
	}
