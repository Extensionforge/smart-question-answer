...
class ASQA_QA_Query_Hooks {
	...
	public static function sql_filter( $sql, $wp_query ) {
		...
		if ( isset( $wp_query->query['asqa_query'] ) ) {
			...
			if ( isset( $wp_query->query['asqa_current_user_ignore'] ) && false === $wp_query->query['asqa_current_user_ignore'] ) {
			...
			$asqa_order_by  = isset( $wp_query->query['asqa_order_by'] ) ? $wp_query->query['asqa_order_by'] : 'active';
			$answer_query = isset( $wp_query->query['asqa_answers_query'] );

			if ( 'answers' === $asqa_order_by && ! $answer_query ) {
				$sql['orderby'] = 'IFNULL(qameta.answers, 0) DESC, ' . $sql['orderby'];
			} elseif ( 'views' === $asqa_order_by && ! $answer_query ) {
				$sql['orderby'] = 'IFNULL(qameta.views, 0) DESC, ' . $sql['orderby'];
			} elseif ( 'unanswered' === $asqa_order_by && ! $answer_query ) {
				$sql['orderby'] = 'IFNULL(qameta.answers, 0) ASC,' . $sql['orderby'];
			} elseif ( 'voted' === $asqa_order_by ) {
				$sql['orderby'] = 'CASE WHEN IFNULL(votes_net, 0) >= 0 THEN 1 ELSE 2 END ASC, ABS(votes_net) DESC, ' . $sql['orderby'];
			} elseif ( 'unsolved' === $asqa_order_by && ! $answer_query ) {
				$sql['orderby'] = "if( qameta.selected_id = '' or qameta.selected_id is null, 1, 0 ) DESC," . $sql['orderby'];
			} elseif ( 'oldest' === $asqa_order_by ) {
			...
			$sql = apply_filters( 'asqa_qa_sql', $sql );
	...
	public static function posts_results( $posts, $instance ) {
		...
		foreach ( (array) $posts as $k => $p ) {
			if ( in_array( $p->post_type, array( 'question', 'answer' ), true ) ) {
				...
				if ( ! empty( $p_arr['ptype'] ) ) {
					$qameta = asqa_qameta_fields();
				} else {
					$qameta = asqa_get_qameta( $p->ID );
				}
				...
				$p->asqa_qameta_wrapped = true;
				$p->votes_net         = $p->votes_up - $p->votes_down;

				// Unset if user cannot read.
				if ( ! asqa_user_can_read_post( $p, false, $p->post_type ) ) {
		...
		if ( isset( $instance->query['asqa_question_query'] ) || isset( $instance->query['asqa_answers_query'] ) ) {
			$instance->pre_fetch();
		}
	...
	public static function imaginary_post( $p ) {
		$_post = array(
			'ID'           => 0,
			'post_title'   => __( 'No permission', 'smart-question-answer' ),
			'post_content' => __( 'You do not have permission to read this question.', 'smart-question-answer' ),
	...
	public static function modify_main_posts( $posts, $query ) {
		if ( ! is_admin() && $query->is_main_query() && $query->is_search() && 'question' === get_query_var( 'post_type' ) ) {
			...
			$posts                = array( get_post( asqa_opt( 'base_page' ) ) );
		}
	...
	public static function pre_get_posts( $query ) {
		if ( $query->is_single() && $query->is_main_query() && 'question' === get_query_var( 'post_type' ) ) {
			$query->set( 'post_status', array( 'publish', 'trash', 'moderate', 'private_post', 'future', 'asqa_spam' ) );
		}
	}
}
