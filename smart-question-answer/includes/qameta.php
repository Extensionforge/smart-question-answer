<?php
/**
 * Holds asqa_qameta table helpers.
 *
 * @since 4.0.0
 * @package SmartQa
 */
 ...
 * @since   4.0.0
 */
function asqa_insert_qameta( $post_id, $args, $wp_error = false ) {
	if ( empty( $post_id ) ) {
		return $wp_error ? new WP_Error( 'Post ID is required' ) : false;
	}

	$_post  = get_post( $post_id );
	$exists = asqa_get_qameta( $post_id );
	...
	foreach ( (array) asqa_qameta_fields() as $field => $type ) {
		if ( isset( $args[ $field ] ) ) {
			$value = $args[ $field ];
	...
	// Don't insert or update if not SmartQa CPT.
	...
	if ( $exists->is_new ) {
		...
		$inserted = $wpdb->insert( $wpdb->asqa_qameta, $sanitized_values, $formats ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
	} else {
		$inserted = $wpdb->update( $wpdb->asqa_qameta, $sanitized_values, array( 'post_id' => $post_id ), $formats ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery
	}
	...
	return $wp_error ? new WP_Error( __( 'Unable to insert SmartQa qameta', 'smart-question-answer' ) ) : false;
}
 ...
 */
function asqa_delete_qameta( $post_id ) {
	global $wpdb;
	return $wpdb->delete( $wpdb->asqa_qameta, array( 'post_id' => $post_id ), array( '%d' ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery
}
 ...
 * @since  3.1.0
 */
function asqa_get_qameta( $post_id ) {
	global $wpdb;

	$qameta = $wpdb->get_row( $wpdb->prepare( "SELECT qm.*, p.post_type as ptype FROM {$wpdb->posts} p LEFT JOIN {$wpdb->asqa_qameta} qm ON qm.post_id = p.ID WHERE p.ID = %d", $post_id ), ARRAY_A ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery
	...
	$qameta = wp_parse_args( $qameta, asqa_qameta_fields() );
 ...
 * @return object
 */
function asqa_append_qameta( $post ) {
	// Convert object as array to prevent using __isset of WP_Post.
	$post_arr = (array) $post;
	if ( ! in_array( $post_arr['post_type'], array( 'question', 'answer' ), true ) || isset( $post_arr['asqa_qameta_wrapped'] ) ) {
		return $post;
	}

	$exist = true;
	foreach ( asqa_qameta_fields() as $fields_name => $val ) {
	...
	if ( ! $exist ) {
		$defaults = asqa_get_qameta( $post->ID );
		if ( ! empty( $defaults ) ) {
 ...
 * @since  3.1.0
 */
function asqa_update_answers_count( $question_id, $counts = false, $update_time = true ) {
	if ( false === $counts ) {
		$counts = asqa_count_published_answers( $question_id );
	}
	...
	return asqa_insert_qameta( $question_id, $args );
}
 ...
 * @since  3.1.0
 */
function asqa_update_votes_count( $post_id ) {
	$count = asqa_count_post_votes_by( 'post_id', $post_id );
	asqa_insert_qameta( $post_id, $count );
	return $count;
}
 ...
 */
function asqa_set_selected_answer( $question_id, $answer_id ) {
	...
	if ( asqa_opt( 'close_selected' ) ) {
		$q_args['closed'] = 1;
	}

	asqa_insert_qameta( $question_id, $q_args );
	$ret = asqa_update_answer_selected( $answer_id );

	$_post = asqa_get_post( $answer_id );
	 ...
	 */
	do_action( 'asqa_select_answer', $_post, $question_id );
 ...
 * @return integer|false
 * @since  3.1.0
 * @since  4.1.2 Insert activity to `asqa_activity` table.
 * @since  4.1.8 Reopen question after unselecting.
 */
function asqa_unset_selected_answer( $question_id ) {
	$qameta = asqa_get_qameta( $question_id );
	...
	$_post = asqa_get_post( $qameta->selected_id );
	 ...
	 * @since 4.1.8 Moved from ajax-hooks.php.
	 */
	do_action( 'asqa_unselect_answer', $_post, $question_id );
 ...
 * @since  3.1.0
 */
function asqa_update_views_count( $post_id, $views = false ) {
	if ( false === $views ) {
		$qameta = asqa_get_qameta( $post_id );
		$views  = (int) $qameta->views + 1;
	}

	asqa_insert_qameta( $post_id, array( 'views' => $views ) );
	return $views;
}
 ...
 * @since  3.1.0
 */
function asqa_update_last_active( $post_id ) {
	return asqa_insert_qameta( $post_id, array( 'last_updated' => current_time( 'mysql' ) ) );
}
 ...
 * @since  3.1.0
 */
function asqa_set_flag_count( $post_id, $count = 1 ) {
	return asqa_insert_qameta( $post_id, array( 'flags' => $count ) );
}
 ...
 * @since  3.1.0
 */
function asqa_update_flags_count( $post_id ) {
	$count = asqa_count_post_flags( $post_id );
	asqa_insert_qameta( $post_id, array( 'flags' => $count ) );
 ...
 * @since  3.1.0
 */
function asqa_update_answer_selected( $answer_id, $selected = true ) {
	return asqa_insert_qameta( $answer_id, array( 'selected' => (bool) $selected ) );
}
 ...
 * @since  3.1.0
 */
function asqa_update_subscribers_count( $post_id, $count = false ) {
	if ( false === $count ) {
		$count = asqa_subscribers_count( 'question', $post_id );
	}

	asqa_insert_qameta( $post_id, array( 'subscribers' => $count ) );
 ...
 * @since  3.1.0
 */
function asqa_update_qameta_terms( $question_id ) {
	$terms = array();
	...
	if ( ! empty( $term_ids ) ) {
		asqa_insert_qameta( $question_id, array( 'terms' => $term_ids ) );
	}
 ...
 * @return boolean
 */
function asqa_set_featured_question( $post_id ) {
	return asqa_insert_qameta( $post_id, array( 'featured' => 1 ) );
}
 ...
 * @return boolean
 */
function asqa_unset_featured_question( $post_id ) {
	return asqa_insert_qameta( $post_id, array( 'featured' => 0 ) );
}
 ...
 * @return array
 */
function asqa_update_post_attach_ids( $post_id ) {
	global $wpdb;
	...
	$insert = asqa_insert_qameta( (int) $post_id, array( 'attach' => $ids ) );
	return $ids;
}
 ...
 * @return boolean|integer
 */
function asqa_update_post_activities( $post_id, $activities = array() ) {
	return asqa_insert_qameta(
		$post_id,
		array(
 ...
 * @return boolean
 * @since  2.4.7
 * @deprecated 4.1.2  Use @see asqa_activity_add(). Activities are inserted in `asqa_activity` table.
 */
function asqa_update_post_activity_meta( $post, $type, $user_id, $append_to_question = false, $date = false ) {
	if ( empty( $post ) ) {
		return false;
	}
	...
	if ( 'answer' === $post_o->post_type && $append_to_question ) {
		$_post         = asqa_get_post( $post_o->post_parent );
		$meta          = $_post->activities;
		$meta['child'] = $meta_val;
		asqa_update_post_activities( $post_o->post_parent, $meta );
	}

	return asqa_update_post_activities( $post_o->ID, $meta_val );
}
 ...
 * @return boolean
 */
function asqa_toggle_close_question( $post_id ) {
	$qameta = asqa_get_qameta( $post_id );
	$toggle = $qameta->closed ? 0 : 1;
	asqa_insert_qameta( $post_id, array( 'closed' => $toggle ) );
 ...
 */
function asqa_get_post_field( $field, $_post = null ) {
 ...
 */
function asqa_post_field( $field = null, $_post = null ) {
	echo asqa_get_post_field( $field, $_post ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
}
 ...
 * @since 4.1.13
 */
function asqa_update_user_unpublished_count( $user_id = null ) {
	global $wpdb;
	...
	update_user_meta( $user_id, '__asqa_unpublished_questions', $q_count );

	// Update answers count.
	update_user_meta( $user_id, '__asqa_unpublished_answers', $a_count );
}
