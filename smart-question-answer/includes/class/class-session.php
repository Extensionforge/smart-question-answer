<?php
/**
 * SmartQa session handler for managing custom session for users.
 *
...
/**
 * SmartQa session handler for managing custom session
...
class Session {
	...
	public function __construct() {
		if ( isset( $_COOKIE[ $this->name ] ) ) {
			$this->id = stripslashes( asqa_sanitize_unslash( $_COOKIE[ $this->name ] ) ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
		} else {
	...
	/**
	 * Get an offset by from SmartQa session.
	 *
	...
	public function get( $key ) {
		$cache = get_transient( 'smartqa_session_' . $this->id );
		if ( false === $cache ) {
			return;
		}
	...
	public function set( $key, $val = null ) {
		...
		set_transient( 'smartqa_session_' . $this->id, $cache, $this->expires );
	}
	 ...
	 * @return void
	 * @since 1.0.0
	 * @since 1.0.0 Fixed: new answers are not being set in session.
	 */
	...
	public function delete( $key = null ) {
		...
		if ( null === $key ) {
			delete_transient( 'smartqa_session_' . $this->id );
			return;
		}
