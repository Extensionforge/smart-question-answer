 ...
 * The activity query.
 *
 * Query wrapper for fetching and parsing SmartQa activities. In actual
 ...
 * @since 4.1.2
 */
class Activity extends SmartQa_Query {
	...
	public function query() {
		...
		if ( ! empty( $this->args['exclude_roles'] ) ) {
			...
			if ( ! empty( $role_like ) ) {
				...
				$sql['where'][] = "AND ( um.meta_key = '{$casqa_key}' AND ( {$role_like} ) )";
			}
		}

		$where = implode( ' ', $sql['where'] );

		$query = "SELECT SQL_CALC_FOUND_ROWS {$sql['fields']} FROM {$wpdb->asqa_activity} a $exclude WHERE 1=1 {$where} ORDER BY {$sql['orderby']} {$sql['order']} LIMIT {$this->offset},{$this->per_page}";
	...
	public function get_the_verb() {
		...
		return sprintf( $verb, asqa_user_display_name( $this->get_user_id() ) );
	}

	/**
	 * Echo current activity verb.
	 *
	 * This is a wrapper method for SmartQa\Activity::get_the_verb().
	 *
	 * @return void
	 * @since 4.1.2
	 */
	 ...
	 *
	 * This is a wrapper method for @see SmartQa\Activity::get_the_icon().
	 *
	 * @return void
	 * @since 4.1.2
	 */
	...
	public function the_ref_content( $show_question = false ) {
		include asqa_get_theme_location( 'activities/activity-ref-content.php' );
	}
	...
	public function when( $object ) {
		...
		if ( $date >= strtotime( '-30 minutes' ) ) {
			$when = __( 'Just now', 'smart-question-answer' );
		} elseif ( $date >= strtotime( '-24 hours' ) ) {
			$when = __( 'Today', 'smart-question-answer' );
		} elseif ( $date >= strtotime( '-48 hours' ) ) {
			$when = __( 'Yesterday', 'smart-question-answer' );
	 ...
	 *
	 * This is a wrapper for method @see SmartQa\Activity::when() but it
	...
	public function the_when() {
		...
		echo '<div class="asqa-activity-when">' . esc_html( $current_when ) . '</div>';
	}
	...
	public function more_button() {
		...
		$args = wp_json_encode(
			array(
				'asqa_ajax_action' => 'more_activities',
				'__nonce'        => wp_create_nonce( 'load_activities' ),
				'paged'          => $this->paged + 1,
			)
		);

		echo '<a href="#" class="asqa-btn" apajaxbtn apquery="' . esc_js( $args ) . '">' . esc_attr__( 'Load More', 'smart-question-answer' ) . '</a>';
	}
}
