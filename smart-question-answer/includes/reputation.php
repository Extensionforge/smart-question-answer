 ...
 * @since 4.0.0
 */
function asqa_insert_reputation( $event, $ref_id, $user_id = false ) {

	// Don't do insert notification if defined.
	if ( defined( 'ASQA_DISABLE_INSERT_REP' ) && ASQA_DISABLE_INSERT_REP ) {
		return;
	}
	...
	// Update user meta.
	asqa_update_user_reputation_meta( $user_id );

	/**
	 * Trigger action after inserting a reputation.
	 */
	do_action( 'asqa_insert_reputation', $new_id, $user_id, $event, $ref_id );
 ...
 * @return array
 */
function asqa_get_reputation( $event, $ref_id, $user_id = false ) {
	global $wpdb;
	...
	$reputation = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $wpdb->asqa_reputations WHERE rep_user_id = %d AND rep_ref_id = %d AND rep_event = %s", $user_id, $ref_id, $event ) );  // phpcs:ignore WordPress.DB
 ...
 * @since 4.0.0
 */
function asqa_delete_reputation( $event, $ref_id, $user_id = false ) {
	global $wpdb;
	 ...
	 */
	do_action( 'asqa_delete_reputation', $deleted, $user_id, $event );
 ...
 * @since 4.0.0
 */
function asqa_register_reputation_event( $event_slug, $args ) {
	$args = wp_parse_args(
		$args,
		array(
	...
	$custom_points                               = get_option( 'smartqa_reputation_events' );
	$args['points']                              = isset( $custom_points[ $event_slug ] ) ? (int) $custom_points[ $event_slug ] : (int) $args['points'];
	smartqa()->reputation_events[ $event_slug ] = $args;
}
 ...
 * @since 4.0.0
 */
function asqa_get_reputation_events() {
	if ( ! empty( smartqa()->reputation_events ) ) {
		do_action( 'asqa_reputation_events' );
	}

	return smartqa()->reputation_events;
}
 ...
 * @since 4.0.0
 */
function asqa_get_reputation_event_points( $event ) {
	$events = asqa_get_reputation_events();
 ...
 * @since 4.0.0
 */
function asqa_get_reputation_event_icon( $event ) {
	$events = asqa_get_reputation_events();
 ...
 * @since 4.0.0
 */
function asqa_get_reputation_event_activity( $event ) {
	$events = asqa_get_reputation_events();
 ...
 * @since 4.0.0
 */
function asqa_get_user_reputation( $user_id, $group = false ) {
	global $wpdb;

	$events = $wpdb->get_results( $wpdb->prepare( "SELECT count(*) as count, rep_event  FROM {$wpdb->asqa_reputations} WHERE rep_user_id = %d GROUP BY rep_event", $user_id ) ); // phpcs:ignore WordPress.DB
 ...
 * @since 4.0.0
 */
function asqa_get_user_reputation_meta( $user_id = false, $short = true ) {
	if ( false === $user_id ) {
		$user_id = get_current_user_id();
	}

	$meta = get_user_meta( $user_id, 'asqa_reputations', true ); // @codingStandardsIgnoreLine.

	if ( false === $short ) {
		return $meta;
	}

	return asqa_short_num( $meta );
}
 ...
 * @param integer|bool $user_id User id.
 */
function asqa_update_user_reputation_meta( $user_id = false ) {
	if ( false === $user_id ) {
		$user_id = get_current_user_id();
	}

	update_user_meta( $user_id, 'asqa_reputations', asqa_get_user_reputation( $user_id ) ); // @codingStandardsIgnoreLine
}
 ...
 * @since  4.0.0
 */
function asqa_get_users_reputation( $user_ids ) {
	global $wpdb;
	$user_counts = array();
	...
	$query     = "SELECT count(*) as count, rep_event, rep_user_id FROM {$wpdb->asqa_reputations} WHERE rep_user_id IN ({$sanitized}) GROUP BY rep_event, rep_user_id";
	...
	$counts     = array();
	$all_events = asqa_get_reputation_events();
 ...
 * @since 4.3.0
 */
function asqa_insert_reputation_event( $slug, $label, $description, $points, $activity, $parent = '' ) {
	global $wpdb;

	$slug     = sanitize_key( $slug );
	$existing = asqa_get_reputation_event_by_slug( $slug );
	...
	if ( $inserted ) {
		...
		// Delete cache.
		wp_cache_delete( 'all', 'asqa_get_all_reputation_events' );
		 ...
		 * @since 4.3.0
		 */
		do_action( 'asqa_inserted_reputation_event', $inserted_id );
 ...
 * @since 4.3.0
 */
function asqa_get_reputation_event_by_slug( $slug ) {
	global $wpdb;

	return $wpdb->get_row( // phpcs:ignore WordPress.DB.DirectDatabaseQuery
		$wpdb->prepare( "SELECT * FROM $wpdb->asqa_reputation_events WHERE slug = %s LIMIT 1", $slug )
	);
}
 ...
 * @since 4.3.0
 */
function asqa_delete_reputation_event_by_slug( $slug ) {
	global $wpdb;

	$event = asqa_get_reputation_event_by_slug( $slug );
	...
	if ( $rows ) {
		 ...
		 * @since 4.3.0
		 */
		do_action( 'asqa_deleted_reputation_event', $event );
 ...
 * @since 4.3.0
 */
function asqa_get_all_reputation_events( $args = array() ) {
	global $wpdb;
	...
	$query = "SELECT * FROM $wpdb->asqa_reputation_events WHERE 1=1 $order_st $limit_st";
 ...
 * @since 4.0.0
 */
class SmartQa_Reputation_Query {
	/**
	 * The loop iterator.
	 *
	 * @access public
	...
	public function __construct( $args = array() ) {
		$this->events = asqa_get_reputation_events();
	...
	private function query() {
		...
		$query = $wpdb->prepare( "SELECT SQL_CALC_FOUND_ROWS * FROM {$wpdb->asqa_reputations} WHERE rep_user_id = %d{$not_in} ORDER BY rep_date {$order} LIMIT %d,%d", $this->args['user_id'], $this->offset, $this->per_page ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared

		$result            = $wpdb->get_results( $query ); // phpcs:ignore WordPress.DB
		$this->total_count = $wpdb->get_var( apply_filters( 'asqa_reputations_found_rows', 'SELECT FOUND_ROWS()', $this ) ); // phpcs:ignore WordPress.DB
	...
	public function have() {
		if ( $this->current + 1 < $this->count ) {
			...
			do_action( 'asqa_reputations_loop_end' );
	...
	public function the_reputation() {
		...
		if ( 0 === $this->current ) {
			 ...
			 */
			do_action( 'asqa_reputation_loop_start' );
		}
	}
	...
	public function get_points() {
		return asqa_get_reputation_event_points( $this->reputation->rep_event );
	}
	...
	public function the_date() {
		echo esc_attr( asqa_human_time( $this->get_date(), false ) );
	}
	...
	public function get_icon() {
		return asqa_get_reputation_event_icon( $this->reputation->rep_event );
	}
	...
	public function get_activity() {
		return asqa_get_reputation_event_activity( $this->reputation->rep_event );
	}
	...
	public function the_ref_content() {
		if ( in_array( $this->reputation->parent, array( 'post', 'question', 'answer' ), true ) ) {
			echo '<a class="asqa-reputation-ref" href="' . esc_url( asqa_get_short_link( array( 'asqa_p' => $this->reputation->rep_ref_id ) ) ) . '">';
			...
			if ( ! empty( $this->reputation->ref->post_content ) ) {
				echo '<p>' . esc_html( asqa_truncate_chars( wp_strip_all_tags( $this->reputation->ref->post_content ), 200 ) ) . '</p>';
			}

			echo '</a>';
		} elseif ( 'comment' === $this->reputation->parent ) {
			echo '<a class="asqa-reputation-ref" href="' . esc_url( asqa_get_short_link( array( 'asqa_c' => $this->reputation->rep_ref_id ) ) ) . '">';
			if ( ! empty( $this->reputation->ref->comment_content ) ) {
				echo '<p>' . esc_html( asqa_truncate_chars( wp_strip_all_tags( $this->reputation->ref->comment_content ), 200 ) ) . '</p>';
			}
			echo '</a>';
		}
	}
