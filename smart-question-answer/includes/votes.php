...
class SmartQa_Vote {
	...
	public static function vote() {
		...
		if ( ! asqa_verify_nonce( 'vote_' . $post_id ) ) {
				asqa_ajax_json( 'something_wrong' );
		}

		$type   = 'vote_up' === asqa_sanitize_unslash( 'type', 'request' ) ? 'vote_up' : 'vote_down';
		$value  = 'vote_up' === $type ? '1' : '-1';
		$userid = get_current_user_id();
		$post   = asqa_get_post( $post_id );
		$thing  = asqa_user_can_vote_on_post( $post_id, $type, $userid, true );
		...
		if ( 'question' === $post->post_type && asqa_opt( 'disable_down_vote_on_question' ) && 'vote_down' === $type ) {
			asqa_ajax_json( 'voting_down_disabled' );
		} elseif ( 'answer' === $post->post_type && asqa_opt( 'disable_down_vote_on_answer' ) && 'vote_down' === $type ) {
			asqa_ajax_json( 'voting_down_disabled' );
		}

		$is_voted = asqa_get_vote( $post_id, get_current_user_id(), 'vote' );
		...
		if ( false !== $is_voted ) {
			...
			if ( $is_voted->vote_value == $value ) { // phpcs:ignore WordPress.PHP.StrictComparisons.LooseComparison
				$counts = asqa_delete_post_vote( $post_id, $userid, 'vote_up' === $type );
				asqa_ajax_json(
					array(
						'success'   => true,
						'action'    => 'undo',
						'vote_type' => $type,
						'snackbar'  => array(
							'message' => __( 'Your vote has been removed.', 'smart-question-answer' ),
						),
						'voteData'  => array(
			...
			asqa_ajax_json(
				array(
					'success'  => false,
					'snackbar' => array(
						'message' => __( 'Undo your vote first.', 'smart-question-answer' ),
					),
					'voteData' => array(
						'active' => $type,
						'nonce'  => wp_create_nonce( 'vote_' . $post_id ),
		...
		$counts = asqa_add_post_vote( $post_id, $userid, 'vote_up' === $type );
		...
		asqa_ajax_json(
			array(
				...
				'vote_type' => $type,
				'snackbar'  => array(
					'message' => __( 'Thank you for voting.', 'smart-question-answer' ),
				),
				'voteData'  => array(
					'net'    => $counts['votes_net'],
					'active' => $type,
	...
	public static function delete_votes( $post_id ) {
		$votes = asqa_get_votes( array( 'vote_post_id' => $post_id ) );

		foreach ( (array) $votes as $vote ) {
			asqa_delete_post_vote( $vote->vote_post_id, $vote->vote_user_id );
		}
	}
	 ...
	 * @since 4.0.0
	 */
	public static function asqa_deleted_votes( $post_id, $type ) {
		if ( 'vote' === $type ) {
			asqa_update_votes_count( $post_id );
		} elseif ( 'flag' === $type ) {
			asqa_update_flags_count( $post_id );
		}
	}
}

/**
 * Insert vote in asqa_votes table.
 *
 * @param  integer       $post_id Post ID.
 ...
 * @since  4.0.0
 */
function asqa_vote_insert( $post_id, $user_id, $type = 'vote', $rec_user_id = 0, $value = '', $date = false ) {
	if ( false === $date ) {
	...
	$inserted = $wpdb->insert( $wpdb->asqa_votes, $args, array( '%d', '%d', '%d', '%s', '%s', '%s' ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
	...
	if ( false !== $inserted ) {
		 ...
		 * @since  4.0.0
		 */
		do_action( 'asqa_insert_vote', $args );
 ...
 * @since  4.0.0
 */
function asqa_get_votes( $args = array() ) {
	if ( ! is_array( $args ) ) {
		$args = array( 'vote_post_id' => $args );
	}

	global $wpdb;
	$where = "SELECT * FROM {$wpdb->asqa_votes} WHERE 1=1";
 ...
 * @return array|boolean
 */
function asqa_count_votes( $args ) {
	global $wpdb;
	...
	$where .= " FROM {$wpdb->asqa_votes} WHERE 1=1 ";
 ...
 * @return array
 * @since  4.0.0
 * @uses   asqa_count_votes
 */
function asqa_count_post_votes_by( $by, $value ) {
 ...
 * @since  4.0.0
 */
function asqa_get_vote( $post_id, $user_id, $type, $value = '' ) {
	global $wpdb;
	$where = "SELECT * FROM {$wpdb->asqa_votes} WHERE 1=1 ";
 ...
 * @uses   asqa_get_vote
 */
function asqa_is_user_voted( $post_id, $type = 'vote', $user_id = false ) {
	if ( false === $user_id ) {
		$user_id = get_current_user_id();
	}

	if ( false === asqa_get_vote( $post_id, $user_id, $type ) ) {
		return false;
	}
 ...
 * @since  4.0.0
 */
function asqa_delete_vote( $post_id, $user_id = false, $type = 'vote', $value = false ) {
	global $wpdb;
	...
	$row = $wpdb->delete( $wpdb->asqa_votes, $where ); // db call okay, db cache okay.

	if ( false !== $row ) {
		do_action( 'asqa_delete_vote', $post_id, $user_id, $type, $value );
	}
 ...
 * @since  4.0.0
 */
function asqa_add_post_vote( $post_id, $user_id = 0, $up_vote = true ) {
	if ( false === $user_id ) {
		$user_id = get_current_user_id();
	}
	...
	$value = $up_vote ? '1' : '-1';
	$row   = asqa_vote_insert( $post_id, $user_id, 'vote', $rec_user_id, $value );

	if ( false !== $row ) {
		// Update qameta.
		$counts    = asqa_update_votes_count( $post_id );
		$vote_type = $up_vote ? 'vote_up' : 'vote_down';

		/**
			* Action asqa_[vote_type]
			* Action triggred after adding a vote for a post.
			*
			* @param integer $post_id Post ID.
			* @param array   $counts All vote counts.
			*/
		do_action( 'asqa_' . $vote_type, $post_id, $counts );
 ...
 */
function asqa_delete_post_vote( $post_id, $user_id = false, $up_vote = null ) {
	$value = false;
	...
	$rows = asqa_delete_vote( $post_id, $user_id, 'vote', $value );

	if ( false !== $rows ) {
		$counts = asqa_update_votes_count( $post_id );
		do_action( 'asqa_undo_vote', $post_id, $counts );
		do_action( 'asqa_undo_' . $type, $post_id, $counts );
 ...
 * @since 0.1
 */
function asqa_vote_btn( $post = null, $echo = true ) {
	$post = asqa_get_post( $post );
	if ( ! $post || 'answer' === $post->post_type && asqa_opt( 'disable_voting_on_answer' ) ) {
		return;
	}

	if ( 'question' === $post->post_type && asqa_opt( 'disable_voting_on_question' ) ) {
		return;
	}

	$vote  = is_user_logged_in() ? asqa_get_vote( $post->ID, get_current_user_id(), 'vote' ) : false;
	...
	$data = array(
		'post_id' => $post->ID,
		'active'  => $type,
		'net'     => asqa_get_votes_net(),
		'__nonce' => wp_create_nonce( 'vote_' . $post->ID ),
	);

	$html  = '';
	$html .= '<div id="vote_' . $post->ID . '" class="asqa-vote net-vote" asqa-vote="' . esc_js( wp_json_encode( $data ) ) . '">';
	$html .= '<a class="apicon-thumb-up asqa-tip vote-up' . ( $voted ? ' voted' : '' ) . ( $vote && 'vote_down' === $type ? ' disable' : '' ) . '" href="#" title="' . ( $vote && 'vote_down' === $type ? __( 'You have already voted', 'smart-question-answer' ) : ( $voted ? __( 'Withdraw your vote', 'smart-question-answer' ) : __( 'Up vote this post', 'smart-question-answer' ) ) ) . '" ap="vote_up"></a>';
	$html .= '<span class="net-vote-count" data-view="asqa-net-vote" itemprop="upvoteCount" ap="votes_net">' . asqa_get_votes_net() . '</span>';

	if ( ( 'question' === $post->post_type && ! asqa_opt( 'disable_down_vote_on_question' ) ) ||
		( 'answer' === $post->post_type && ! asqa_opt( 'disable_down_vote_on_answer' ) ) ) {
		$html .= '<a data-tipposition="bottom center" class="apicon-thumb-down asqa-tip vote-down' . ( $voted ? ' voted' : '' ) . ( $vote && 'vote_up' === $type ? ' disable' : '' ) . '" href="#" title="' . ( $vote && 'vote_up' === $type ? __( 'You have already voted', 'smart-question-answer' ) : ( $voted ? __( 'Withdraw your vote', 'smart-question-answer' ) : __( 'Down vote this post', 'smart-question-answer' ) ) ) . '" ap="vote_down"></a>';
	}
	 ...
	 * @since 4.1.5
	 */
	$html = apply_filters( 'asqa_vote_btn_html', $html, $post );
 ...
 * @since  4.0.0
 */
function asqa_user_votes_pre_fetch( $ids ) {
	if ( $ids && is_user_logged_in() ) {
		$votes = asqa_get_votes(
			array(
 ...
 * @return boolean
 */
function asqa_delete_votes( $post_id, $type = 'vote' ) {
	global $wpdb;
	$where = array(
		'vote_post_id' => $post_id,
		'vote_type'    => $type,
	...
	$rows = $wpdb->delete( $wpdb->asqa_votes, $where ); // db call okay, db cache okay.

	if ( false !== $rows ) {
		do_action( 'asqa_deleted_votes', $post_id, $type );
		return true;
	}
