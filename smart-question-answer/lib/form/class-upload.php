<?php
/**
 * SmartQa Upload type field object.
 *
 * @package    SmartQa
 * @subpackage Fields
 * @since      4.1.0
...
class Upload extends Field {
	...
	protected function prepare() {
		$this->args = wp_parse_args(
			$this->args,
			array(
				'label'          => __( 'SmartQa Upload Field', 'smart-question-answer' ),
				'upload_options' => array(),
				'browse_label'   => __( 'Select file(s) to upload', 'smart-question-answer' ),
			)
		);
		...
		$this->args['upload_options'] = wp_parse_args(
			...
			array(
				...
				'max_files'       => 1,
				'multiple'        => false,
				'label_deny_type' => __( 'This file type is not allowed to upload.', 'smart-question-answer' ),
				'async_upload'    => false,
			)
		);
		...
		if ( ! isset( $this->args['upload_options']['label_max_added'] ) ) {
			$this->args['upload_options']['label_max_added'] = sprintf(
				...
				__( 'You cannot add more then %d files', 'smart-question-answer' ),
	...
	protected function html_order() {
		if ( empty( $this->args['output_order'] ) ) {
			$this->output_order = array( 'wrapper_start', 'label', 'field_wrasqa_start', 'errors', 'field_markup', 'desc', 'file_list', 'field_wrasqa_end', 'wrapper_end' );
		} else {
	...
	public function file_list() {
		$medias = get_posts(
			array(
				...
				'title'       => '_asqa_temp_media',
				'post_author' => get_current_user_id(),
			)
		);

		// Show temporary images uploaded.
		$this->add_html( '<div class="asqa-upload-list">' );
	...
	public function field_markup() {
		...
		$this->add_html( '<div class="asqa-upload-c">' );
	...
	public function after_save( $args = array() ) {
		...
		if ( ! empty( $this->uploaded_files ) ) {
			foreach ( $this->uploaded_files as $id ) {
				asqa_set_media_post_parent( $id, $args['post_id'] );
			}
		}
	}
	 ...
	 * All files were uploaded to `smartqa-temp` directory and it need to moved
	 * manually. All files older then 2 hours are deleted from `smartqa-temp` directory.
	 *
	...
	private function upload( $file ) {
		...
		$upload_error_strings = array(
			false,
			__( 'The uploaded file exceeds the upload_max_filesize directive in php.ini.', 'smart-question-answer' ),
			__( 'The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form.', 'smart-question-answer' ),
			__( 'The uploaded file was only partially uploaded.', 'smart-question-answer' ),
			__( 'No file was uploaded.', 'smart-question-answer' ),
			'',
			__( 'Missing a temporary folder.', 'smart-question-answer' ),
			__( 'Failed to write file to disk.', 'smart-question-answer' ),
			__( 'File upload stopped by extension.', 'smart-question-answer' ),
		);
		...
		if ( ! ( $file_size > 0 ) ) {
			return new \WP_Error( 'upload_file_size', __( 'File is empty. Please upload something more substantial.', 'smart-question-answer' ) );
		}

		// Check file size.
		if ( $file_size > asqa_opt( 'max_upload_size' ) ) {
			return new \WP_Error( 'upload_file_size', __( 'File is bigger than the allowed limit.', 'smart-question-answer' ) );
		}

		// Check file uploaded using proper method.
		if ( true !== is_uploaded_file( $file['tmp_name'] ) ) {
			return new \WP_Error( 'upload_file_failed', __( 'Specified file failed upload test.', 'smart-question-answer' ) );
		}
		...
		if ( ! $type || ! $ext ) {
			return new \WP_Error( 'upload_file_ext', __( 'Sorry, this file type is not permitted for security reasons.', 'smart-question-answer' ) );
		}
		...
		$temp_dir = $uploads['basedir'] . '/smartqa-temp/';
	...
	public function get_uploaded_files_url() {
		...
		$uploads  = wp_upload_dir();
		$temp_dir = $uploads['baseurl'] . '/smartqa-temp/';
