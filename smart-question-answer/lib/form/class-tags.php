<?php
/**
 * SmartQa Tags type field object.
 *
 * @package    SmartQa
 * @subpackage Fields
 * @since      4.1.0
...
class Tags extends Field {
	...
	protected function prepare() {
		$this->args = wp_parse_args(
			$this->args,
			array(
				'label'      => __( 'SmartQa Tags Field', 'smart-question-answer' ),
				'array_max'  => 3,
				'array_min'  => 2,
				'terms_args' => array(
		...
		$js_options = array(
			...
			'create'   => false,
			'labelAdd' => __( 'Add', 'smart-question-answer' ),
		);
	...
	public function field_markup() {
		...
		$this->add_html( '<input type="text" id="' . $this->id() . '" data-type="' . $type . '" data-options="' . esc_js( wp_json_encode( $this->get( 'js_options' ) ) ) . '" class="asqa-tags-input" autocomplete="off" aptagfield' . $this->custom_attr() . ' name="' . esc_attr( $this->field_name ) . '" value="' . $value . '" />' );
