<?php
/**
 * SmartQa Editor type field object.
 *
 * @package    SmartQa
 * @subpackage Fields
 * @since      4.1.0
...
class Editor extends Field {
	...
	protected function prepare() {
		$this->args = wp_parse_args(
			$this->args,
			array(
				'label'       => __( 'SmartQa Editor Field', 'smart-question-answer' ),
				'editor_args' => array(
					'quicktags' => false,
				),
			)
	...
	public function image_button() {
		$btn_args = wp_json_encode(
			array(
				'__nonce'   => wp_create_nonce( 'asqa_upload_image' ),
				'action'    => 'asqa_upload_modal',
				'form_name' => $this->form_name,
			)
		);

		$this->add_html( '<button type="button" class="asqa-btn-insertimage asqa-btn-small asqa-btn mb-10 asqa-mr-5" apajaxbtn aponce="false" apquery="' . esc_js( $btn_args ) . '"><i class="apicon-image asqa-mr-3"></i>' . __( 'Insert image', 'smart-question-answer' ) . '</button>' );
		 ...
		 * @since 4.1.8
		 */
		do_action( 'asqa_editor_buttons', $this->original_name, $this );
	}
	...
	public function field_markup() {
		...
		$settings = array(
			'textarea_rows' => 10,
			'tinymce'       => array(
				'content_css'      => asqa_get_theme_url( 'css/editor.css' ),
		 ...
		 * @since 2.0.1
		 */
		$editor_args = apply_filters( 'asqa_pre_editor_settings', $editor_args );

		$this->add_html( '<div class="asqa-editor">' );
		$this->image_button();
		 ...
		 * @since 4.1.8
		 */
		$value = apply_filters( 'asqa_editor_pre_value', $this->value() );
	 ...
	 *
	 * Checks if current image is in `smartqa-temp` directory and
	 * if so then moves it to `smartqa-uploads` directory and return
	...
	public function image_process( $matches ) {
		if ( false === strpos( $matches[1], 'smartqa-temp/' ) ) {
			return $matches[0];
		}

		$files = smartqa()->session->get( 'files' );

		$uploads   = wp_upload_dir();
		$basename  = basename( $matches[1] );
		$temp_file = $uploads['basedir'] . '/smartqa-temp/' . $basename;

		// Check temp file is in smartqa-temp directory.
		if ( ! file_exists( $temp_file ) ) {
			$this->add_error( 'fields-error', __( 'Sorry an error occured while processing your image, please remove it and insert again', 'smart-question-answer' ) );
		} else {
			$upload_dir = $uploads['basedir'] . '/smartqa-uploads/';
			...
			if ( in_array( $basename, $files, true ) ) {
				...
				$new_file_url = $uploads['baseurl'] . "/smartqa-uploads/$basename";
				rename( $uploads['basedir'] . "/smartqa-temp/$basename", $newfile );
	...
	public function after_save( $args = array() ) {
		...
		foreach ( $this->images as $img ) {
			add_post_meta( $args['post_id'], 'smartqa-image', $img );
		}

		// Delete file from session.
		smartqa()->session->delete( 'files' );
	}
