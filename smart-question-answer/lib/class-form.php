...
class Form {
	...
	public function __construct( $form_name, $args ) {
		...
		$this->args      = wp_parse_args(
			...
			array(
				'form_tag'      => true,
				'submit_button' => true,
				'submit_label'  => __( 'Submit', 'smart-question-answer' ),
				'editing'       => false,
				'editing_id'    => 0,
			)
		);
	...
	public function prepare() {
		...
		$fields = asqa_sort_array_by_order( $this->args['fields'] );

		if ( ! asqa_opt( 'allow_private_posts' ) && isset( $fields['is_private'] ) ) {
		...
		foreach ( $fields as $field_name => $field_args ) {
			...
			$field_class = 'SmartQa\\Form\\Field\\' . $type_class;
			...
			if ( class_exists( $field_class ) ) {
				 ...
				 * @since 4.1.0
				 */
				$field_args                  = apply_filters_ref_array( 'asqa_before_prepare_field', array( $field_args, $this ) );
	...
	public function generate( $form_args = array() ) {
		...
		wp_enqueue_script( 'smartqa-upload' );
		...
		if ( empty( $this->args['fields'] ) ) {
			...
			echo esc_attr(
				sprintf(
					...
					esc_attr__( 'No fields found for form: %s', 'smart-question-answer' ),
					$this->form_name
				)
			);
			echo '</p>';
		...
		/**
		 * Allows filtering arguments passed to @see SmartQa\Form\generate() method. Passed
		 * by reference.
		 *
		 ...
		 * @since 4.1.0
		 */
		$form_args = apply_filters_ref_array( 'asqa_generate_form_args', array( $form_args, $this ) );
		...
		// Output form errors.
		if ( $this->have_errors() ) {
			echo '<div class="asqa-form-errors">';
			foreach ( (array) $this->errors as $code => $msg ) {
				echo '<span class="asqa-form-error ecode-' . esc_attr( $code ) . '">' . esc_html( $msg ) . '</span>';
			}
			echo '</div>';
		}
		...
		echo '<input type="hidden" name="asqa_form_name" value="' . esc_attr( $this->form_name ) . '" />';

		if ( true === $form_args['submit_button'] ) {
			echo '<button type="submit" class="asqa-btn asqa-btn-submit">' . esc_html( $this->args['submit_label'] ) . '</button>';
		}
		 ...
		 */
		do_action_ref_array( 'asqa_after_form_field', array( $this ) );
	...
	public function is_submitted() {
		$nonce = asqa_isset_post_value( esc_attr( $this->form_name ) . '_nonce' );

		if ( asqa_isset_post_value( esc_attr( $this->form_name ) . '_submit' ) &&
	...
	private function sanitize_validate( $fields = false ) {
		if ( ! asqa_isset_post_value( $this->form_name . '_submit' ) ) {
			return;
		}
		...
		foreach ( (array) $fields as $field ) {
			...
			if ( true === $field->have_errors() ) {
				$this->add_error( 'fields-error', __( 'Error found in fields, please check and re-submit', 'smart-question-answer' ) );
			}
		}
	}
	 ...
	 * @return void
	 * @since 4.1.0
	 * @since 4.1.5 Delete SmartQa session data.
	 */
	...
	/**
	 * Save current values to SmartQa session so that users unfinished question can
	 * be retrieved later.
	 *
	...
	public function save_values_session( $id = '' ) {
		...
		if ( ! empty( $values ) ) {
			$id = empty( $id ) ? '' : '_' . $id;
			smartqa()->session->set( $this->form_name . $id, $values );
		}
	}
	...
	public function delete_values_session( $id = '' ) {
		$id = empty( $id ) ? '' : '_' . $id;
		smartqa()->session->delete( $this->form_name . $id );
	}
}
