...
class Validate {
	...
	public static function sanitize_wp_kses( $value = null ) {
		if ( null !== $value ) {
			return wp_kses( $value, asqa_form_allowed_tags() );
		}
	}
	 ...
	 * @return null|boolean Return boolean value.
	 */
	public static function sanitize_array_masqa_boolean( $value = null ) {
		if ( ! empty( $value ) ) {
	...
	public static function sanitize_description( $value = null ) {
		if ( ! empty( $value ) ) {
			 ...
			 * @since 4.1.9
			 */
			$new_value = apply_filters( 'asqa_pre_sanitize_description', $value );
	...
	private static function whitelist_shortcodes( $m ) {
		 ...
		 * @since 4.1.8
		 */
		$allowed_shortcodes = apply_filters( 'asqa_allowed_shortcodes', array() );

		// if not allowed shortcode then change square brackets.
		if ( ! in_array( $m[2], $allowed_shortcodes, true ) ) {
			return asqa_replace_square_bracket( $m[0] );
		}
	 ...
	 *
	 * @see `ASQA_Form::sanitize_description`
	 *
	 ...
	 *
	 * @param object $field Instance of @see `ASQA_Field` object.
	 * @return void
	 */
		...
		if ( '' === $field->value() || is_null( $field->value() ) ) {
			$field->add_error(
				...
				sprintf(
					...
					__( '%s field is required.', 'smart-question-answer' ),
					$field->get( 'label' )
				)
			);
		}
	...
	public static function validate_is_email( $field ) {
		if ( ! empty( $field->value() ) && ! is_email( $field->value() ) ) {
			$field->add_error(
				...
				sprintf(
					// Translators: placeholder contain field label.
					__( 'Value provided in field %s is not a valid email.', 'smart-question-answer' ),
					$field->get( 'label' )
				)
			);
		}
	...
	public static function validate_is_url( $field ) {
		if ( ! empty( $field->unsafe_value() ) && false === filter_var( $field->unsafe_value(), FILTER_VALIDATE_URL ) ) {
			$field->add_error(
				...
				sprintf(
					// Translators: placeholder contain field label.
					__( 'Value provided in field %s is not a valid URL.', 'smart-question-answer' ),
					$field->get( 'label' )
				)
			);
		}
	...
	public static function validate_is_numeric( $field ) {
		if ( ! empty( $field->unsafe_value() ) && ! is_numeric( $field->unsafe_value() ) ) {
			$field->add_error(
				...
				sprintf(
					// Translators: placeholder contain field label.
					__( 'Value provided in field %s is not numeric.', 'smart-question-answer' ),
					$field->get( 'label' )
				)
			);
		}
		...
		if ( ! empty( $value ) && $field->get( 'min_length' ) ) {
			...
			if ( mb_strlen( $value, 'utf-8' ) < $min_length ) {
				$field->add_error(
					...
					sprintf(
						// Translators: placeholder contain field label.
						__( 'Value provided in field %1$s must be at least %2$d characters long.', 'smart-question-answer' ),
						$field->get( 'label' ),
						$min_length
					)
				);
		...
		if ( ! empty( $value ) && $field->get( 'max_length' ) ) {
			...
			if ( mb_strlen( $value, 'utf-8' ) > $max_length ) {
				$field->add_error(
					...
					sprintf(
						// Translators: placeholder contain field label.
						__( 'Value provided in field %1$s must not exceeds %2$d characters.', 'smart-question-answer' ),
						$field->get( 'label' ),
						$max_length
					)
				);
	...
	public static function validate_is_array( $field ) {
		...
		if ( ! empty( $value ) && ! is_array( $value ) ) {
			$field->add_error(
				...
				sprintf(
					// Translators: placeholder contain field label.
					__( 'Value provided in field %s is not an array.', 'smart-question-answer' ),
					$field->get( 'label' )
				)
			);
		}
	...
	public static function validate_array_min( $field ) {
		...
		if ( $min_arr > 0 && ( empty( $value ) || ! is_array( $value ) || $min_arr > count( $value ) ) ) {
			$field->add_error(
				...
				sprintf(
					// Translators: placeholder contain field label.
					__( 'Minimum %1$d values are required in field %2$s.', 'smart-question-answer' ),
					$min_arr,
					$field->get( 'label' )
				)
			);
	...
	public static function validate_array_max( $field ) {
		...
		if ( ! empty( $value ) && count( $value ) > $max_arr ) {
			$field->add_error(
				...
				sprintf(
					// Translators: placeholder contain field label.
					__( 'Maximum values allowed in field %2$s is %1$d.', 'smart-question-answer' ),
					$max_arr,
					$field->get( 'label' )
				)
			);
	...
	public static function get_bad_words() {
		$bad_word_file = asqa_get_theme_location( 'badwords.txt' );
	...
	public static function validate_badwords( $field ) {
		...
		if ( ! empty( $found ) ) {
			$field->add_error(
				...
				sprintf(
					// Translators: placeholder contain field label.
					__( 'Found bad words in field %s. Remove them and try again.', 'smart-question-answer' ),
					$field->get( 'label' )
				)
			);
		}
	...
	private static function file_have_error( $field ) {
		...
		$errors = array(
			0 => __( 'There is no error, the file uploaded with success', 'smart-question-answer' ),
			1 => __( 'The uploaded file exceeds the upload_max_filesize directive in php.ini', 'smart-question-answer' ),
			2 => __( 'The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form', 'smart-question-answer' ),
			3 => __( 'The uploaded file was only partially uploaded', 'smart-question-answer' ),
			4 => __( 'No file was uploaded', 'smart-question-answer' ),
			6 => __( 'Missing a temporary folder', 'smart-question-answer' ),
			7 => __( 'Failed to write file to disk.', 'smart-question-answer' ),
			8 => __( 'A PHP extension stopped the file upload.', 'smart-question-answer' ),
		);
	...
	private static function file_size_error( $field ) {
		...
		} elseif ( ! $is_numeric && $value['size'] > asqa_opt( 'max_upload_size' ) ) {
			$have_error = true;
		}
	...
	public static function validate_upload( $field ) {
		...
		if ( ! empty( $value ) ) {
			...
			if ( ! asqa_user_can_upload() ) {
				$field->add_error( 'deny-upload', __( 'You are not allowed to upload file(s)', 'smart-question-answer' ) );
			}
			...
			if (
				...
				$field->add_error(
					...
					sprintf(
						...
						__( 'You cannot upload more than %1$d file in field %2$s', 'smart-question-answer' ),
						$args['max_files'],
						$field->get( 'label' )
					)
				);
			...
			if ( false !== $valid_mimes ) {
				$field->add_error( 'mimes-not-allowed', __( 'File type is not allowed to upload.', 'smart-question-answer' ) );
			}
			...
			if ( false !== $file_size ) {
				$field->add_error(
					...
					sprintf(
						...
						__( 'File(s) size is bigger than %s MB', 'smart-question-answer' ),
						round( asqa_opt( 'max_upload_size' ) / ( 1024 * 1024 ), 2 )
					)
				);
			}
		}
		...
		if ( ! empty( $field ) ) {
			...
			if ( ! $value ) {
				$field->add_error(
					...
					sprintf(
						// Translators: placeholder contain field label.
						__( 'You are required to check %s field', 'smart-question-answer' ),
						$field->get( 'label' )
					)
				);
			}
