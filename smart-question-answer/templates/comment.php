...
<apcomment id="comment-<?php echo (int) $comment->comment_ID; ?>" <?php comment_class( $approved ); ?>>
	<div itemprop="comment" itemscope itemtype="http://schema.org/Comment">
		...
		<div class="asqa-avatar"><?php echo get_avatar( $comment->user_id, $avatar_size ); ?></div>
		<div class="comment-inner">
			<div class="comment-header">
				<a href="<?php echo esc_url( asqa_user_link( $comment->user_id ) ); ?>" class="asqa-comment-author" itemprop="creator" itemscope itemtype="http://schema.org/Person">
					<span itemprop="name"><?php echo esc_html( asqa_user_display_name( $comment ) ); ?></span>
				</a> <?php esc_attr_e( 'commented', 'smart-question-answer' ); ?>

				<a href="<?php comment_link(); ?>" class="asqa-comment-time">
					<time itemprop="dateCreated" datetime="<?php echo esc_attr( gmdate( 'c', strtotime( $comment->comment_date ) ) ); ?>"><?php echo esc_attr( asqa_human_time( $comment->comment_date_gmt, false ) ); ?></time>
				</a>

				<div class="asqa-comment-actions">
					<?php foreach ( asqa_comment_actions( $comment ) as $comment_action ) : ?>
			...
			<?php if ( ! is_single() ) : ?>
				<div class="asqa-comment-content" itemprop="text"><?php comment_text(); ?></div>
			<?php else : ?>
				<?php
					$comment_text = wp_strip_all_tags( get_comment_text() );
					$comment_c    = strlen( $comment_text );
				?>
				<div class="asqa-comment-content" itemprop="text">
					<?php comment_text(); ?>
				</div>
			<?php endif; ?>
		</div>
